...
const Cell = React.memo(({ value, onChange, onFocus }) => {
...
const PointConrols = React.memo(({ point, onChange, onFocus }) => (
  <PointWrapper>
...
const Curve = ({ curve, onChange }) => {
  const [focus, setFocused] = useField("6-focused-curve");
  ...
  const updateCurCurve = React.useCallback(
    (value, i, d) => {
      let points = (curve.points?.map((point) => ({ ...point })) || []);
      points[i][d] = value;
...
export const CurvePoints = ({
  ...
  const [curves, setValue] = useField(fieldName, defaultValue);
  const [, setFocused] = useField("6-focused-curve");
